...
The lock file is created exclusively and carries the owning pid, so a lock left
behind by a crash can be recognised and taken over instead of blocking the
conductor forever. A process-local registry complements it, because a pid alone
cannot distinguish "my own crashed run left this file" from "another instance
inside this very process is holding it right now".
"""
...
class FileRuntimeLock:
    ...
    def acquire(self) -> bool:
        ...
        with _LOCAL_GUARD:
            ...
            # The recorded owner is gone: a previous conductor died without
            # releasing. Reclaiming here is what keeps a crash from permanently
            # disabling orchestration on this machine.
